...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict):
        ...
        num_groups = point["batch"].max().item() + 1
        scenes = torch.zeros(num_groups, point["feat"].size(-1), device=point["feat"].device)
        counts = torch.zeros(num_groups, device=point["feat"].device)
        ones = torch.ones_like(point["batch"], dtype=point["feat"].dtype, device=point["feat"].device)
